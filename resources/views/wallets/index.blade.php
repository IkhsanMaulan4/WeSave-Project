...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-3">
    <div>
        <h2 class="fw-bold text-dark mb-1">Dompet Saya</h2>
        <p class="text-muted">Kelola semua sumber dana Anda di satu tempat.</p>
    </div>
    <button class="btn btn-success fw-bold px-4 py-2 rounded-4 d-flex align-items-center gap-2 shadow-sm" data-bs-toggle="modal" data-bs-target="#addWalletModal" style="background-color: #13ec5b; border: none; color: #000;">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-end position-relative" style="z-index: 1;">
        <div>
            <div class="d-flex align-items-center gap-2 text-muted mb-2">
                ...
                <span class="fw-bold small">Total Saldo Aktif</span>
            </div>
...
<div class="d-flex align-items-center gap-2 mb-4">
    <h5 class="fw-bold text-dark mb-0">Daftar Dompet</h5>
    <span class="badge bg-light text-muted border rounded-pill">{{ $wallets->count() }} Akun</span>
</div>
...
<div class="row g-4">
        ...
        <div class="card card-custom h-100 border-0 p-3 position-relative group-hover">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    ...
                    <form action="{{ route('wallets.destroy', $wallet->id) }}" method="POST" onsubmit="return confirm('Hapus dompet ini?');">
                        @csrf
                        @method('DELETE')
                ...
                <div class="mb-3">
                    <small class="text-muted fw-bold">Saldo Saat Ini</small>
    ...
    <div class="col-md-6 col-xl-4">
        <button class="btn btn-outline-secondary w-100 h-100 d-flex flex-column align-items-center justify-content-center p-5 rounded-4 border-2 border-dashed" data-bs-toggle="modal" data-bs-target="#addWalletModal" style="border-style: dashed !important; min-height: 250px;">
            ...
            <h5 class="fw-bold text-dark">Tambah Dompet Baru</h5>
            <small class="text-muted">Bank, E-Wallet, atau Tunai</small>
        </button>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 p-3">
            <div class="modal-header border-0">
                <h5 class="modal-title fw-bold">Tambah Dompet</h5>
            ...
            <form action="{{ route('wallets.store') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control rounded-3 py-2" placeholder="Contoh: BCA Utama, Gopay Jajan" required>
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer border-0">
                    ...
                    <button type="submit" class="btn btn-success rounded-3 fw-bold px-4" style="background-color: #13ec5b; border: none; color: #000;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
